async function fetchRevenueHeads() {
    ...
    const response = await fetch(`${HOST}/?getAllSecondary`)
  ...
  console.log(revenues)
    if (revenues.status === 0) {
        ...
        revenues.message.forEach((userInvoice, i) => {
            ...
            addd += `
              <td>
                <a href="./view-secondary.html?id=${userInvoice.id}&load=true" target="_blank" class="btn btn-primary btn-sm viewUser" >View Details</a>
              </td>
              </tr>
              `
